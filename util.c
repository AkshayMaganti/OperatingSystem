...
const char *CPIN = "cpin";
const char *CPOUT = "cpout";
...
void handleCommand(char **argc){
    ...
    else if(strcmp(argc[0],CPIN) == 0){//call the cpin method
        cpin(argc[1], argc[2]);
    }
    else if(strcmp(argc[0], CPOUT) == 0){//cpout
        cpout(argc[1], argc[2]);
    }
    else if(strcmp(argc[0], MKDIR) == 0){//mkdir
        mkdir(argc[1]);
    }
    else if(strcmp(argc[0], RM) == 0){//rm
        rm(argc[1]);
    }
